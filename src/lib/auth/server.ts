...
import { safeTanstackCookies } from "./safe-cookies.server";
import {
  GROK_ISSUER_DEFAULT,
...
const baseURL = {
  allowedHosts: [...previewAllowedHosts, "localhost", "127.0.0.1", "[::1]", ...publicHosts],
...
const grokOAuthPlugin = authConfigured
  ? genericOAuth({
      config: GROK_PROVIDERS.map(({ providerId, idp }) => ({
        ...
        // Sempre mostra a lista de contas (Google/X) em vez de entrar
        // sozinho com a última sessão do navegador.
        prompt: "select_account",
        authorizationUrlParams: { idp, prompt: "select_account" },
      })),
    })
  : null;
...
export const auth = betterAuth({
  ...
  rateLimit: {
    ...
    customRules: {
      ...
      "/sign-in/oauth2": { window: 60, max: 30 },
    },
  },
